...
#reho.data <- returnPercentileGroup('me', reho.data$F1_Exec_Comp_Cog_Accuracy, reho.data)
#alff.data <- returnPercentileGroup('me', alff.data$F1_Exec_Comp_Cog_Accuracy, alff.data)
#ad.data <- returnPercentileGroup('me', ad.data$F1_Exec_Comp_Cog_Accuracy, ad.data)
#rd.data <- returnPercentileGroup('me', rd.data$F1_Exec_Comp_Cog_Accuracy, rd.data)
#tr.data <- returnPercentileGroup('me', tr.data$F1_Exec_Comp_Cog_Accuracy, tr.data)
#fa.data.label <- returnPercentileGroup('me', fa.data.label$F1_Exec_Comp_Cog_Accuracy, fa.data.label)
#tr.data.label <- returnPercentileGroup('me', tr.data.label$F1_Exec_Comp_Cog_Accuracy, tr.data.label)
...
# Now do the same procedure in only the modality regressed data 
# Load the data
# Now we need to load the data 
...
fa.data.label <- read.csv('/home/adrose/dataPrepForHiLoPaper/data/meanLRVolandAgeRegModalReg/jhuFALabelsData.csv')
tr.data.label <- read.csv('/home/adrose/dataPrepForHiLoPaper/data/meanLRVolandAgeRegModalReg/jhuTRLabelsData.csv')
...
# Now produce the importance value for our volume data
pdf('varImpPlotsTop20MR.pdf', height=30, width=20)
male.vol.data <- vol.data[which(vol.data$sex==1),]
vol.col <- grep('mprage_jlf_vol', names(vol.data))
male.vol.values <- scale(male.vol.data[,vol.col])[,1:length(vol.col)]
male.vol.outcome <- scale(male.vol.data$F1_Exec_Comp_Cog_Accuracy)[,1]
...
output <- as.data.frame(output)
output <- reshape(data=output, timevar='tmp', idvar='V3', v.names='output', direction='wide')
write.csv(output, 'maleRFImportanceModalReg.csv', quote=F, row.names=F)
...
output <- as.data.frame(output)
output <- reshape(data=output, timevar='tmp', idvar='V3', v.names='output', direction='wide')
write.csv(output, 'maleRFImportanceModalReg2.csv', quote=F, row.names=F)
...
output <- as.data.frame(output)
output <- reshape(data=output, timevar='tmp', idvar='V3', v.names='output', direction='wide')
write.csv(output, 'femaleRFImportanceModalReg.csv', quote=F, row.names=F)
...
output <- as.data.frame(output)
output <- reshape(data=output, timevar='tmp', idvar='V3', v.names='output', direction='wide')
write.csv(output, 'femaleRFImportanceModalReg2.csv', quote=F, row.names=F)
