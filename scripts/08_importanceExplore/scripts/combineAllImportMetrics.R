...
pdf('scaleRidgeMal.pdf', width=16, height=16)
ggpairs(data=all.data,columns=c(2,6:7),ggplot2::aes(colour=modality))
ggpairs(data=all.data[grep("mprage_jlf_vol", all.data$ROI_readable),],columns=c(2,6:7),ggplot2::aes(colour=modality))
dev.off()
...
pdf('scaleRidgeFem.pdf', width=16, height=16)
ggpairs(data=all.data,columns=c(2,6:7),ggplot2::aes(colour=modality))
ggpairs(data=all.data[grep("mprage_jlf_vol", all.data$ROI_readable),],columns=c(2,6:7),ggplot2::aes(colour=modality))
dev.off()
