    ...
    for (null in c("No", "Yes")) {
      ...
      for (run in 1:1000) {
        ...
        if (a < 9) {
          ...
          x_train_df <- thisdf[train, c("bblid", "age", "age2", "age3", xvars)]
          y_train_df <- thisdf[train, c("bblid", yvar)]
          x_test_df <- thisdf[test, c("bblid", "age", "age2", "age3", xvars)]
...
write.csv(results_df, file="~/Documents/hiLo/data/permutationResults_half_Age_strongNull_OLSNoPenalizeAge.csv", row.names=FALSE)

toPlotVals <- summarySE(data=results_df[,c('Modality', 'Sex', 'Null', 'RSq')],
  groupvars=c('Modality', 'Sex', 'Null'), measurevar='RSq')

write.csv(toPlotVals, file="~/Documents/hiLo/data/permutationSummary_half_Age_strongNull_OLSNoPenalizeAge.csv", row.names=FALSE)
...
png(file="~/Documents/hiLo/plots/figure7_color_Age_strongNull_OLSNoPenalizeAge.png", height=160, width=120, units='mm', res=800)
out.plot
dev.off()

png(file="~/Documents/hiLo/plots/figure7_color_Age_strongNull_OLSNoPenalizeAge_histogram.png", height=160, width=120, units='mm', res=800)
out.plot_histogram
dev.off()
