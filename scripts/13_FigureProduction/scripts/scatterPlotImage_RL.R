...
# Load data
df <- read.csv("/home/butellyn/age_prediction/data/n1601_imagingclinicalcognitive_20190130.csv")
...
nback_quality_df <- nback_quality_df[nback_quality_df$scanid %in% df$scanid, ]

nback_df <- nback_df[nback_df$scanid %in% df$scanid, ]
...
# Apply rest exclusions to nback data... suboptimal
nback.modal.data <- merge(nback.modal.data, df[,c("bblid", "restExclude")])
nback.modal.data <- nback.modal.data[nback.modal.data$restExclude == 0, ]
rownames(nback.modal.data) <- 1:nrow(nback.modal.data)

# Make other dataframes
vol.modal.data <- df[,c("bblid", "scanid", "ageAtScan1", "F1_Exec_Comp_Cog_Accuracy", "averageManualRating", "t1Exclude", grep("mprage_jlf_vol_", colnames(df), value=TRUE))]
vol.modal.data <- vol.modal.data[vol.modal.data$t1Exclude == 0, ]
rownames(vol.modal.data) <- 1:nrow(vol.modal.data)
gmd.modal.data <- 
tr.modal.data <- 
cbf.modal.data <- 
alff.modal.data <- 
...
for (q in 1:7) {
	...
	if (length(unique(sum_df$Importance)) == 3) { thesecolors <- c("grey70", "grey50", "black")
	} else if (length(unique(sum_df$Importance)) == 2) { thesecolors <- c("grey70", "black") }

	plot_fun <- function(sum_df, lowerbound, upperbound, thesecolors) {
		force(lowerbound)
		force(upperbound)
		ggplot(sum_df, aes(x=Male, y=Female)) + theme_minimal() + #geom_rect fails if limits not hardcoded 
		...
		geom_point(aes(color=Importance, shape=Importance)) + 
		geom_text_repel(aes(label=Abbrev2, color=Importance), size=6, nudge_x=c(minscale/5, maxscale/5), nudge_y=c(minscale/8, maxscale/8)) +
		guides(text_repel=FALSE) +
		scale_color_manual(values=thesecolors) + labs(title=thistit) + #, subtitle=subtit) + 
		...
		geom_hline(yintercept=-thisSE*2, linetype="dashed", color="slateblue") + geom_vline(xintercept=-thisSE*2, linetype="dashed", color="slateblue") + 
		annotate(geom="text", x=minscale+maxscale/8, y=maxscale, size=6, label=paste0("r = ", round(cor(sum_df$Female, sum_df$Male), digits=3))) +
		theme(plot.title = element_text(size=15, face="bold"), axis.title = element_text(size=15), axis.text = element_text(size=15))
	}
	
	outPlot <- plot_fun(sum_df, lowerbound, upperbound, thesecolors)
	
	if (maxscale == .4) {
		outPlot <- outPlot + scale_x_continuous(limits=c(minscale, maxscale), breaks=seq(minscale, maxscale, .1)) + 
		scale_y_continuous(limits=c(minscale, maxscale), breaks=seq(minscale, maxscale, .1)) 
	} else {
		outPlot <- outPlot + scale_x_continuous(limits=c(minscale, maxscale), breaks=seq(minscale+.05, maxscale-.05, .1)) + 
		scale_y_continuous(limits=c(minscale, maxscale), breaks=seq(minscale+.05, maxscale-.05, .1)) 
	}
