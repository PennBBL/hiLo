...
sum_df$Modality <- factor(sum_df$Modality, levels = c("Volume", "GMD", "MD", "NBack", "IdEmo", "CBF", "ALFF", "ReHo"))
sum_df$Importance <- ordered(sum_df$Importance, levels=c("PF", "Other"))
sum_df$Group <- paste(sum_df$Sex, sum_df$ageBin)
sum_df$Group <- ordered(sum_df$Group, levels=c("Female Young Adults", "Female Adolescents", "Female Children", "Male Young Adults", "Male Adolescents", "Male Children"))
sum_df$Sex <- factor(sum_df$Sex)

thisplot <- ggplot(sum_df, aes(Abbrev, EffectSize, group=Group, colour=Group)) +
	geom_point(aes(shape=Group), stat="identity") +
	facet_nested(Modality ~ Lobe + Importance, scales="free", space="free_x") +
	scale_shape_manual(values=c(17, 15, 16, 17, 15, 16)) +
	scale_color_manual(values=c("red3", "violetred1", "pink", "blue4", "steelblue2", "lightsteelblue1")) +
...
if (galton == TRUE) {
	pdf(paste0('/home/butellyn/hiLo/plots/beta_HiLo_Age_condensed_', Sys.Date(), '.pdf'), width=15, height=13)
	thisplot
	dev.off()
} else if (mymachine == TRUE) {
	pdf(paste0('/Users/butellyn/Documents/hiLo/plots/beta_HiLo_Age_condensed_', Sys.Date(), '.pdf'), width=15, height=13)
	thisplot
	dev.off()
}
