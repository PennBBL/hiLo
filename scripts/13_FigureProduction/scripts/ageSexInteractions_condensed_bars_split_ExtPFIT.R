...
install_load('plyr', 'ggplot2', 'reshape2', 'grid', 'gridExtra', 'labeling',
	'data.table', 'ggrepel', 'DescTools', 'gtable', 'ggh4x', 'dplyr')
...
front <- c('FO', 'MFC', 'MOrG', 'POrG', 'OrIFG', 'TrIFG', 'AOrG', 'OpIFG', 'GRe', 'FRP', 'LOrG', 'PrG', 'MSFG', 'SMC', 'MFG', 'SFG', 'MPrG')
temporal <- c('FuG', 'PT', 'PP', 'ITG', 'CO', 'MTG', 'TMP', 'STG', 'TTG')
parietal <- c('PCu', 'PoG', 'AnG', 'PO', 'SPL', 'SMG', 'MPoG')
...
for (a in 1:8) {
  ...
  sum_df_tmp$Sex <- c(rep('Female', length(colstouse)*3), rep('Male', length(colstouse)*3))
	...
	ExtPFITregions_exlamation <- c('OrIFG', 'POrG', 'Hipp', 'OFuG', 'AnG', 'Thal', 'TMP')
	ExtPFITregions_question <- c('MSFG', 'SMG', 'NA')
...
sum_df$ageBin <- ordered(sum_df$ageBin, levels=c('Children', 'Adolescents', 'Adults'))
sum_df$Sex <- factor(sum_df$Sex)
sum_df$Sex <- recode(sum_df$Sex, 'Female'='F', 'Male'='M')

struc_plot <- ggplot(sum_df[sum_df$Modality %in% c('Volume', 'GMD', 'MD') &
		sum_df$Lobe %in% c('Baso-Striatal', 'Limbic', 'Frontal', 'Temporal', 'Parietal', 'Occipital'), ],
		aes(Abbrev, EffectSize, group=ageBin, colour=ageBin, fill=ageBin)) +
	geom_bar(stat='identity', position='dodge') + scale_y_continuous(limits=c(-1.2, 1.2), #########
		breaks=round(seq(-1.5, 1.5, .5), digits=1)) +
	facet_nested(Modality ~ Lobe + Importance + Sex, scales='free', space='free_x') +
	...
	labs(fill='Age Bin', colour='Age Bin') + theme(axis.text.x = element_text(angle=90)) +
	...
	scale_color_manual(values=c('black', 'black', 'black'), guide = guide_legend(nrow=1)) +
	scale_fill_manual(values=c('white', 'gray62','black'), guide = guide_legend(nrow=1))

func_plot <- ggplot(sum_df[sum_df$Modality %in% c('CBF', 'ALFF', 'ReHo') &
		sum_df$Lobe %in% c('Baso-Striatal', 'Limbic', 'Frontal', 'Temporal', 'Parietal', 'Occipital'), ],
		aes(Abbrev, EffectSize, group=ageBin, colour=ageBin, fill=ageBin)) +
...
task_plot <- ggplot(sum_df[sum_df$Modality %in% c('NBack', 'IdEmo') &
		sum_df$Lobe %in% c('Baso-Striatal', 'Limbic', 'Frontal', 'Temporal', 'Parietal', 'Occipital'), ],
		aes(Abbrev, EffectSize, group=ageBin, colour=ageBin, fill=ageBin)) +
...
wmcer_plot <- ggplot(wmcer_df, aes(Abbrev, EffectSize, group=ageBin, colour=ageBin, fill=ageBin)) +
	geom_bar(stat='identity', position='dodge') + scale_y_continuous(limits=c(-.8, 1.2), breaks=round(seq(-1.4, 1.4, .2), digits=1)) +
	facet_nested(. ~ Lobe + Modality + Sex, scales='free', space='free_x') +
	scale_shape_manual(values=c(17, 15, 16, 17, 15, 16)) +
	scale_color_manual(values=c('black', 'black', 'black'), guide = guide_legend(nrow=1)) +
	scale_fill_manual(values=c('white', 'gray62','black'), guide = guide_legend(nrow=1)) +
	ylab('Effect Size') + theme_linedraw() + geom_hline(yintercept=0) +
	...
	labs(fill = 'Age Bin', colour='Age Bin') + theme(axis.text.x = element_text(angle=90)) +
	theme(legend.position='bottom', legend.box='vertical', axis.title.x=element_blank(),
		panel.spacing=unit(.1, 'lines'), axis.text.x = element_text(angle=45, hjust=1, size=6))
...
if (galton == TRUE) {
	...
	tiff('~/Documents/hiLo/plots/figure3.tif', units='mm', width=240, height=150, res=300)
	struc_plot
	dev.off()

	tiff('~/Documents/hiLo/plots/figure4.tif', units='mm', width=240, height=150, res=300)
	func_plot
	dev.off()

	tiff('~/Documents/hiLo/plots/figure5.tif', units='mm', width=240, height=150, res=300)
	task_plot
	dev.off()

	tiff('~/Documents/hiLo/plots/figure6.tif', units='mm', width=240, height=100, res=300)
	wmcer_plot
	dev.off()
