...
###
### Ellyn Butler
### December 12, 2019 - December 20, 2019
...
thisplot <- ggplot(sum_df, aes(Abbrev, EffectSize, group=Group, colour=Group, fill=Group)) +
...
if (galton == TRUE) {
	pdf(paste0('/home/butellyn/hiLo/plots/beta_HiLo_Age_condensed_bars_', Sys.Date(), '.pdf'), width=12, height=8)
	thisplot
	dev.off()
} else if (mymachine == TRUE) {
	pdf(paste0('/Users/butellyn/Documents/hiLo/plots/beta_HiLo_Age_condensed_bars_', Sys.Date(), '.pdf'), width=12, height=8)
	thisplot
	dev.off()
}
