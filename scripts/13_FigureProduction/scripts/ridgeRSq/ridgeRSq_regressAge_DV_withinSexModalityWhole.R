  ...
  for (sex in c(2, 1)) {
    ...
    for (perm in c("No", "Yes")) {
      ...
      for (run in 1:1000) {
        ...
        thismod <- lm(F1_Exec_Comp_Res_Accuracy ~ age + age2 + age3, data=thisdf)
        thisdf[,"F1_Exec_Comp_Res_Accuracy"] <- thismod$residuals
...
write.csv(results_df, file="~/Documents/hiLo/data/permutationResults_half_regressAge_DV_withinSexModalityWhole.csv", row.names=FALSE)

toPlotVals <- summarySE(data=results_df[,c('Modality', 'Sex', 'Permuted', "RSq")],
  groupvars=c('Modality', 'Sex', 'Permuted'), measurevar='RSq')

write.csv(toPlotVals, file="~/Documents/hiLo/data/permutationSummary_half_regressAge_DV_withinSexModalityWhole.csv", row.names=FALSE)
...
png(file="~/Documents/hiLo/plots/figure7_color_regressAge_DV_withinSexModalityWhole.png", height=160, width=120, units='mm', res=800)
out.plot
dev.off()
