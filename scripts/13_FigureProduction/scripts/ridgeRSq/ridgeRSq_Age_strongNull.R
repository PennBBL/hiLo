...
write.csv(results_df, file="~/Documents/hiLo/data/permutationResults_half_Age_strongNull.csv", row.names=FALSE)

toPlotVals <- summarySE(data=results_df[,c('Modality', 'Sex', 'Permuted', "RSq")],
  groupvars=c('Modality', 'Sex', 'Permuted'), measurevar='RSq')

write.csv(toPlotVals, file="~/Documents/hiLo/data/permutationSummary_half_Age_strongNull.csv", row.names=FALSE)
...
png(file="~/Documents/hiLo/plots/figure7_color_Age_strongNull.png", height=160, width=120, units='mm', res=800)
out.plot
dev.off()
