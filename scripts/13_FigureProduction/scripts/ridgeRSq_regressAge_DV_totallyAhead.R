...
demo.data <- read.csv("~/Documents/n9498/n9498_demographics_go1_20161212.csv") #using ageAtCnb1
cog.data <- merge(cog.data, demo.data)
cog.data <- cog.data[!is.na(cog.data$F1_Exec_Comp_Res_Accuracy),]
rownames(cog.data) <- 1:nrow(cog.data)
cog.data <- cog.data[, c("bblid", "ageAtCnb1", "F1_Exec_Comp_Res_Accuracy")]

# Create age vars
cog.data$age <- scale(cog.data$ageAtCnb1)
cog.data$age2 <- scale((cog.data$age)^2)
cog.data$age3 <- scale((cog.data$age)^3)

# Regress out age from the cognitive variable
thismod <- lm(F1_Exec_Comp_Res_Accuracy ~ age + age2 + age3, data=cog.data)
cog.data[,"F1_Exec_Comp_Res_Accuracy"] <- thismod$residuals
...
write.csv(results_df, file="~/Documents/hiLo/data/permutationResults_half_regressAge_DV_totallyAhead.csv", row.names=FALSE)

toPlotVals <- summarySE(data=results_df[,c('Modality', 'Sex', 'Permuted', "RSq")],
  groupvars=c('Modality', 'Sex', 'Permuted'), measurevar='RSq')

write.csv(toPlotVals, file="~/Documents/hiLo/data/permutationSummary_half_regressAge_DV_totallyAhead.csv", row.names=FALSE)
...
png(file="~/Documents/hiLo/plots/figure7_color_regressAge_DV_totallyAhead.png", height=160, width=120, units='mm', res=800)
out.plot
dev.off()
