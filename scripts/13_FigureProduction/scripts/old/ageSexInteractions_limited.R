...
for (a in 1:8) {
  ...
  sum_df$Sex <- c(rep("Female", length(colstouse)*3), rep("Male", length(colstouse)*3))
    ...
    sum_df[c(i+length(colstouse)*2, i+length(colstouse)*5), "ageBin"] <- "Young Adults"
      ...
      sum_df[seq(i, i+length(colstouse)*5, length(colstouse)), "Lobe"] <- "BasGang"
      ...
      sum_df[seq(i, i+length(colstouse)*5, length(colstouse)), "Lobe"] <- "Cer"
		} else if (intname %in% whitematter) {
      sum_df[seq(i, i+length(colstouse)*5, length(colstouse)), "Lobe"] <- "White"
    }
	}
	...
	sum_df$Lobe <- factor(sum_df$Lobe, levels = c("BasGang", "Limbic", "Frontal", "Temporal", "Parietal", "Occipital"))
	for (j in 1:nrow(sum_df)) {
		if (sum_df[j, "Abbrev"] %in% PFITregions) {
			sum_df[j, "Importance"] <- "PF"
		} else {
      sum_df[j, "Importance"] <- "Not"
    }
	}
  ...
  if (data.names[a] == "vol") {
  abbrevtokeep <- c()
    for (lobe in levels(sum_df$Lobe)) {
      pick_df <- sum_df[sum_df$ageBin == "Young Adults" & sum_df$Lobe == lobe & sum_df$Importance == "Not",]
      ...
      if (lobe %in% c("BasGang", "Limbic", "Parietal", "Occipital", "Cer", "White")) { numother <- 2
      } else { numother <- 3 }
  ...
  sum_df <- sum_df[sum_df$Importance == "PF" | sum_df$Abbrev %in% abbrevtokeep,]
  ...
  sum_df$Sex <- factor(sum_df$Sex)
	...
	assign(paste0(data.names[a], "_plot"), thisplot)
}

if (galton == TRUE) {
	pdf(paste0('/home/butellyn/hiLo/plots/beta_HiLo_Age_limited_', Sys.Date(), '.pdf'), width=12, height=12)
	grid.arrange(vol_plot, gmd_plot, nback_plot, idemo_plot, nrow=2, ncol=2)
  grid.arrange(tr_plot, cbf_plot, alff_plot, reho_plot, nrow=2, ncol=2)
	dev.off()
} else if (mymachine == TRUE) {
	png(paste0('/Users/butellyn/Documents/hiLo/plots/beta_HiLo_Age_limited_struc_', Sys.Date(), '.png'), width=6, height=18, units='in', res=300)
  grid.arrange(vol_plot, gmd_plot, tr_plot, nrow=3)
  dev.off()

  png(paste0('/Users/butellyn/Documents/hiLo/plots/beta_HiLo_Age_limited_physio_', Sys.Date(), '.png'), width=6, height=18, units='in', res=300)
  grid.arrange(cbf_plot, alff_plot, reho_plot, nrow=3)
  dev.off()

  png(paste0('/Users/butellyn/Documents/hiLo/plots/beta_HiLo_Age_limited_task_', Sys.Date(), '.png'), width=6, height=12, units='in', res=300)
  grid.arrange(nback_plot, idemo_plot, nrow=2)
	dev.off()
}
