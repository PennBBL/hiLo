    ...
    for (null in c("No", "Yes")) {
      ...
      for (run in 1:1000) {
        ...
        if (a < 9) {
          ...
          x_train_df <- thisdf[train, c("bblid", qualitymetrics[a], xvars)]
          y_train_df <- thisdf[train, c("bblid", yvar)]
          x_test_df <- thisdf[test, c("bblid", qualitymetrics[a], xvars)]
          y_test_df <- thisdf[test, c("bblid", yvar)]
        } else {
          train_df <- thisdf[train, ]
          x_train_df <- thisdf[train, c("bblid", unique(qualitymetrics), xvars)]
          y_train_df <- thisdf[train, c("bblid", yvar)]
          x_test_df <- thisdf[test, c("bblid", unique(qualitymetrics), xvars)]
          y_test_df <- thisdf[test, c("bblid", yvar)]
        }
...
write.csv(results_df, file="~/Documents/hiLo/data/permutationResults_half_QA_strongNull_OLSNoPenalizeQA.csv", row.names=FALSE)

toPlotVals <- summarySE(data=results_df[,c('Modality', 'Sex', 'Null', 'RSq')],
  groupvars=c('Modality', 'Sex', 'Null'), measurevar='RSq')

write.csv(toPlotVals, file="~/Documents/hiLo/data/permutationSummary_half_QA_strongNull_OLSNoPenalizeQA.csv", row.names=FALSE)
...
png(file="~/Documents/hiLo/plots/figure7_color_QA_strongNull_OLSNoPenalizeQA.png", height=160, width=120, units='mm', res=800)
out.plot
dev.off()

png(file="~/Documents/hiLo/plots/figure7_color_QA_strongNull_OLSNoPenalizeQA_histogram.png", height=160, width=120, units='mm', res=800)
out.plot_histogram
dev.off()
