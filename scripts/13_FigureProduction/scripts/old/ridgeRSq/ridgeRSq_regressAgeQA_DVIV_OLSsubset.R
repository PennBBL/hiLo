  ...
  for (sex in c(2, 1)) {
    ...
    for (perm in c("No", "Yes")) {
      ...
      for (run in 1:1000) {
        ...
        # ONLY USE FIRST FIVE XVARS FOR SANITY CHECK
        xvars <- xvars[1:5]
        ...
        # Build the OLS model
        finalfunc <- paste0(yvar, " ~ ", xvars[1], "+", xvars[2], "+", xvars[3], "+", xvars[4], "+", xvars[4])
        ols_model <- lm(formula(finalfunc), data=train_df)
...
write.csv(results_df, file="~/Documents/hiLo/data/permutationResults_half_regressAgeQA_DVIV_OLSsubset.csv", row.names=FALSE)

toPlotVals <- summarySE(data=results_df[,c('Modality', 'Sex', 'Permuted', "RSq")],
  groupvars=c('Modality', 'Sex', 'Permuted'), measurevar='RSq')

write.csv(toPlotVals, file="~/Documents/hiLo/data/permutationSummary_half_regressAgeQA_DVIV_OLSsubset.csv", row.names=FALSE)
...
png(file="~/Documents/hiLo/plots/figure7_color_regressAgeQA_DVIV_OLSsubset.png", height=160, width=120, units='mm', res=800)
out.plot
dev.off()

png(file="~/Documents/hiLo/plots/figure7_color_regressAgeQA_DVIV_histogram_OLSsubset.png", height=160, width=120, units='mm', res=800)
out.plot_histogram
dev.off()
