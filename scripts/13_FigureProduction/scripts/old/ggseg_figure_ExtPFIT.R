...
new_data_F$PFIT <- as.character(new_data_F$PFIT)
new_data_M$PFIT <- as.character(new_data_M$PFIT)
new_data_F$Count <- as.numeric(new_data_F$Count)
new_data_M$Count <- as.numeric(new_data_M$Count)
for (i in 1:nrow(new_data_F)) {
  # Adult Females
  area <- as.character(new_data_F[i, "area"])
  if (area %in% PFITregions) { pfitstat <- "PFIT"
  } else if (area %in% ExtPFITregions) { pfitstat <- "Ext PFIT"
  } else { pfitstat <- "Other" }
...
final_data$Count2 <- recode(final_data$Count2, "0 Other"="Other", "1 Other"="Other",
  "2 Other"="Other", "3 Other"="Other", "4 Other"="Other", "5 Other"="Other",
  "6 Other"="Other")
final_data$Count2 <- factor(final_data$Count2, levels=c("0 Ext PFIT", "1 Ext PFIT",
  "2 Ext PFIT", "3 Ext PFIT", "4 Ext PFIT", "5 Ext PFIT", "6 Ext PFIT", "0 PFIT",
  "1 PFIT", "2 PFIT", "3 PFIT", "4 PFIT", "5 PFIT", "6 PFIT", "Other"))
...
png(file="/Users/butellyn/Documents/hiLo/plots/figure2_color_ExtPFIT.png", units="mm", width=180, height=100, res=800)
p
dev.off()
