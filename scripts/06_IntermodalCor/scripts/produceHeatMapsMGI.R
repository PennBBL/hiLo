...
cc.data <- read.csv('/home/adrose/dataPrepForHiLoPaper/data/meanLRMGI/ccData.csv') 

# Now merge all of the data
all.data <- merge(vol.data, gmd.data, by=intersect(names(vol.data), names(gmd.data)), all=T)
...
grepPattern1 <- c('mprage_jlf_vol_', 'mprage_jlf_gmd_', 'mprage_jlf_ct_','mprage_jlf_cortcon_')
